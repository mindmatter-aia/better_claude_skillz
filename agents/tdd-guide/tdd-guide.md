...
describe('searchItems', () => {
  it('returns semantically similar items', async () => {
    const results = await searchItems('electronics')

    expect(results).toHaveLength(5)
    expect(results[0].name).toContain('Laptop')
    expect(results[1].name).toContain('Tablet')
  })
})
```
...
describe('GET /api/items/search', () => {
  it('returns 200 with valid results', async () => {
    const request = new NextRequest('http://localhost/api/items/search?q=test+query')
  ...
  it('falls back to substring search when cache unavailable', async () => {
    // Mock cache failure
    jest.spyOn(cache, 'searchItemsByVector').mockRejectedValue(new Error('Cache down'))
...
test('user can search and view item', async ({ page }) => {
  ...
  // Search for item
  await page.fill('input[placeholder="Search items"]', 'electronics')
...
```typescript
jest.mock('@/lib/cache', () => ({
  searchItemsByVector: jest.fn(() => Promise.resolve([
